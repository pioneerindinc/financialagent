...
## Local validation results (September 14, 2026)
...
This is a preparation/runbook, not deployment authorization. No PIONEERSERV settings, services, databases or environment files were inspected or changed. Hostnames, ports, service account, service name, centralized environment path and release roots below must be supplied/approved by the platform owner. Commands are for a future approved deployment only.

## Source and release contract

- Repository: `https://github.com/pioneerindinc/financialagent.git`.
- Branch: `main`; audit baseline: `85e6acb`.
...
`.env.example` documents names only; blank values are not a runnable production configuration. Store populated values in the centralized ACL-protected file, outside releases and source control.
...
| `APP_BASE_URL`                       | Required for human authentication/mutation configuration. Canonical external HTTPS origin, including nonstandard port if applicable. Mutation Origin must exactly match its origin. Source schema accepts a URL; enforce HTTPS operationally.                                                           |
| `AUTH_ISSUER`                        | Required for human sessions. Exact JWT issuer URL from the trusted Pioneer identity bridge.                                                                                                                                                                                                             |
| `AUTH_AUDIENCE`                      | Required for human sessions. Nonempty app-specific audience, agreed with the issuer.                                                                                                                                                                                                                    |
| `AUTH_JWKS_URL`                      | Required for human sessions. HTTPS endpoint containing the trusted RS256 public verification keys; PIONEERSERV must reach it and validate its TLS chain.                                                                                                                                                |
...
3. **Interactive accounting staging:** additionally configure all four human auth variables (`APP_BASE_URL`, issuer, audience, JWKS), a real trusted session-issuance bridge, company assignments, and separately approved bootstrap. No production auth bypass is available. A short-lived, company-scoped `finance:read` service token can test read APIs after bootstrap, but cannot provide a browser session or post/reverse journals.

## Authentication audit

`src/lib/auth/session.ts` validates RS256 JWTs against configured HTTPS JWKS, exact issuer/audience, mandatory sub/iat/exp, and maximum token age of one hour. Claims must contain a subject, company ID array and recognized finance scopes. The cookie must be `__Host-finance-session`, Secure, HttpOnly, Path=/, no Domain, with appropriate SameSite protection, set by a trusted same-origin identity bridge. No login/callback bridge is implemented here: configuring issuer/JWKS variables alone does not create sign-in.

Human financial writes require finance:write, human principal type and company membership, with exact mutation Origin verification. Use finance:read for viewers. The identity bridge must assign claims server-side and must use the permanent company IDs below. Apply the platform's MFA/access-review policy; revocation is currently bounded by session expiry unless the bridge adds a revocation mechanism.
...
Use an explicit database name in the URI (proposed staging name: `financialagent_staging`, subject to DBA approval). Confirm actual member addresses, replicaSet option, TLS/authSource, firewall reachability, primary selection and dedicated least-privilege runtime credentials. Health probing does not grant privileges or create this database.
...
No `use server` Server Action/Server Function declarations are present in application source. Mutations use Route Handlers. Human JWT verification uses public JWKS; there is no application session-minting secret for Pioneer auth.
...
Local evidence: `node_modules/next/dist/docs/01-app/02-guides/self-hosting.md` (Server Functions encryption key / version skew) and `01-app/01-getting-started/15-route-handlers.md`. No additional Next.js production runtime secret is required by the current application.
...
- Real Pioneer session issuer/bridge, application audience, reachable JWKS, exact company assignments and external canonical origin for interactive acceptance. Dev auth must remain unavailable.
