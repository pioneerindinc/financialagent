...
Manual draft creation/edit and normal posting require active company-owned posting accounts with allowManualPosting not false. Posting rechecks current policy, so restricting an account blocks pre-existing manual drafts. Source drafts may use active posting nonmanual/control accounts; inactive accounts are rejected both on source creation and final posting. Source drafts cannot be manually edited, even if their sourceSystem is labelled manual. Source identity/hash deduplication is unchanged: retrying a previously accepted event returns that existing record even after account deactivation, but cannot newly post a blocked draft.

The existing reversal service is a system correction path (`postingOrigin=system`): human finance:write, same-company posted original, exact opposite lines, one reversal only, required reason and open target period. It deliberately permits historical inactive/nonmanual/nonposting accounts because it accepts no caller-supplied replacement lines. This is not a general system-journal posting bypass. Journal balancing, idempotency, source identity, posted immutability and period behavior are unchanged.
...
## Posting headers and explicit Pioneer 2027 setup

See [Pioneer 2027 accounting setup](../PIONEER_2027_ACCOUNTING_SETUP.md) for the authoritative postingAccount rules, pending-draft conversion gate, historical reversal exception, explicit transactional period.setup2027 operation and approved cutover boundary. Legacy postingAccount defaults to true without rewriting records. Headers reject new manual and source lines regardless of control/manual policy. The COA hierarchy is organizational, not a report rollup. The batch period action is human-only, creates missing 2027 calendar months OPEN, preserves exact existing months and rejects every conflicting overlap atomically. It never runs at startup or activates accounting authority.
